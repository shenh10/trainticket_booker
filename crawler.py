...
class Ticket(object):
    ...
    def config_parser(self):
        self.retrieve_station_dict()
        self.retrieve_book_options()
    ...
    def retrieve_book_options(self):
        ...
        self.trains = book_settings['trains'].split(',')
        ...
        if book_settings['student'] == 'Y':
            self.isStudent = True
        self.tolerance = book_settings['tolerance']
            ...
            if tds and len(tds) >= 10:
                ...
                if len(has_ticket) != 0:
                    ...
                    for cat in self.ticket_type:
                        if cat in ticket_index:
                            i = ticket_index.index(cat)
                        else:
                            return 0, ''
                        if tds[i].text != u'无' and tds[i].text != '--':
                            if tds[i].text != u'有':
                                ...
                                print u'%s 的 %s 有余票若干张!'%(this_train, cat ,tds[i].text)
                            self.find_ticket = 1
                            tds.last.click()
                            break
            if self.find_ticket:
    ...
    def book(self):
        ...
        person = []
        while len(person) == 0:
            person=b.find_by_text(persons[0])
        for person in self.people:
            b.find_by_text(person).click()
        return 1
